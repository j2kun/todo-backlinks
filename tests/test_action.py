...
def test_closed_issue_delete_comment_and_recreate():
    bot_comment = (
        "This issue has 1 outstanding TODOs:\n\n"
        " - [myfile.py:123]"
        "(https://github.com/j2kun/todo-backlinks/blob/main/myfile.py#L123)"
    ...
    assert len(actual) == 1
    assert actual[1].delete_comment
    assert "[!NOTE]" in actual[1].new_comment_body
    assert "was closed with unresolved TODOs" in actual[1].new_comment_body

def test_closed_issue_ignore_after_note_added():
    bot_comment = (
        "> [!NOTE]\n"
